...
int main(void) {
    static const UINT8 digest[32] = {
        0xE4,0xAB,0xF0,0xA0,0xB2,0xD2,0xD9,0xD2,0x36,0x16,0x2E,0x45,0xC5,0x82,0x47,0xEA,
        0xBD,0xAC,0x7D,0x5C,0xFB,0x66,0x06,0xA1,0xC7,0x31,0x07,0xC7,0x7A,0xC6,0xEE,0x72
    };
    static const UINT8 signature[LITEOS_RSA2048_BYTES] = {
    ...
    UINT8 altered[32];
    for (unsigned i = 0; i < sizeof(altered); ++i) altered[i] = digest[i];
    if (!rsa2048_sha256_verify(digest, signature)) return 1;
    altered[0] ^= 1U;
    if (rsa2048_sha256_verify(altered, signature)) return 2;
    puts("rsa: ok");
    return 0;
}
