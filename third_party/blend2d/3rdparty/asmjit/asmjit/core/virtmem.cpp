...
static Error alloc_dual_mapping_using_file(Out<DualMapping> dm, size_t size, MemoryFlags memory_flags) noexcept {
  ...
  AnonymousMemory anon_mem;
  Error err = anon_mem.open(prefer_tmp_over_dev_shm);
  if (err != Error::kOk)
    return err;

  err = anon_mem.allocate(size);
  if (err != Error::kOk)
    return err;
  ...
  for (uint32_t i = 0; i < 2; i++) {
    ...
    err = map_memory(&ptr[i], size, restricted_memory_flags | MemoryFlags::kMapShared, anon_mem.fd(), 0);
    if (err != Error::kOk) {
      if (i == 1) {
