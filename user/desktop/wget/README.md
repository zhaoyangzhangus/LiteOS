...
HTTPS uses the vendored OpenSSL 3.5.8 TLS 1.2/1.3 client and can be used
with the bundled ISRG Root X1 anchor; the certificate date is derived from
the build timestamp because the current image has no wall-clock syscall.
...
Certificate validation is fail-closed when the bundled trust anchor, hardware
entropy, or build-time validation date is unavailable; `--no-check-certificate`
deliberately disables certificate and hostname checks for controlled/test
endpoints. FTP currently implements
