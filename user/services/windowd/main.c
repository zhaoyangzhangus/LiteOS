...
static void draw_text(int32_t x, int32_t y, const char *text, uint32_t color) {
    liteos_text_draw(g_frame, g_stride, g_width, g_height,
                     x, y, text, color);
}
...
static bool handle_input(const os_input_event_t *event) {
    ...
    if (event->type == OS_INPUT_EVENT_KEY) {
        if (event->code == 0xE0U || event->code == 0xE4U) {
            g_ctrl = event->value != OS_INPUT_VALUE_RELEASE;
            return true;
        }
        if (event->value != OS_INPUT_VALUE_RELEASE && g_ctrl &&
            event->code == 0x2EU) {
