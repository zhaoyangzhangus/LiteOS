...
static LITEOS_RUN_QUEUE *g_scheduler_queue;
/*
...
UINT64 liteos_lapic_timer_interrupt(LITEOS_INTERRUPT_CONTEXT *context) {
    ...
    lapic_schedule_timer_maintenance();
    UINT64 next_context = 0;
    if (g_scheduler_queue != 0) {
        next_context = liteos_scheduler_timer_tick(g_scheduler_queue, context);
    }
