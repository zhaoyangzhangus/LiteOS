 ...
 */
/*
 * V3.10.6B9E-DIAG2 RESOURCE FREE STAGES
 *
 * Temporary diagnostics:
 *
 *   681 descriptor_buffer
 *   682 hid_report
 *   683 hid_ring
 *   684 audio_buffer
 *   685 audio_ring
 *   686 bt_acl_out_ring
 *   687 bt_acl_out_buffer
 *   688 bt_acl_in_buffer
 *   689 bt_acl_in_ring
 *   690 bt_event_buffer
 *   691 bt_event_ring
 *   692 hub_report
...
static bool xhci_free_device_resources_device(
    ...
    {
        /*
         * Keep the FIRST resource-free failure.
         */
        if(released)
        {
            g_xhci_error =
 ...
 */
/*
 * V3.10.6B9E-DIAG1 REENUM FAILURE STAGES
 *
 * 661 = invalid root port
 * 662 = supplied Slot.context is not canonical
 * 663 = state->device unexpectedly occupied
 * 664 = published subtree teardown failed
 * 665 = physical root port lost CCS after teardown
 */
    ...
    xhci_device_context_t *device)
{
    ...
    {
        /*
         * Preserve DIAG2's exact DMA/resource failure.
         */
        if(g_xhci_error < 681U ||
           g_xhci_error > 696U)
        {
            g_xhci_error =
                664U;
    ...
    uint8_t *child_slot)
{
    ...
    *child_slot =
    ...
    {
        /*
         * Repair stale parent topology while we are here.
         */
        parent->child_slots[
            parent_port - 1U] =
            0U;
    ...
    /*
     * Verify the reverse topology relation too.
     *
     * Slot reuse must never make an old child_slots[] entry
     * point at an unrelated device.
     */
    ...
    /*
     * Only fully published devices count.
     */
    if(child->context.device_slot !=
       slot)
    {
        return false;
    }
 ...
 */
static bool xhci_queue_bt_event(
    xhci_state_t *state)
{
    if(state == 0)
    {
...
static bool xhci_queue_bt_acl_in(
    xhci_state_t *state)
{
    if(state == 0)
    {
...
static bool xhci_enumerate_device(xhci_state_t *state, uint8_t slot,
    ...
    /*
     * V3.4.2 parent child link
     *
     * Build the hardware Slot topology in software.
     */
    if(parent_slot != 0U &&
       parent_port != 0U &&
       parent_port <= 16U)
    {
        xhci_slot_device_t *parent_dev =
            &g_xhci_slot_devices[parent_slot];


        if(parent_dev->used)
        {
            parent_dev->child_slots[parent_port-1U] =
                slot;
        }
    }
