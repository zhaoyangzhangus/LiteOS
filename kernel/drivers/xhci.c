...
#define XHCI_MAX_USB_INVENTORY    16U
...
typedef struct xhci_inventory_entry {
    xhci_device_context_t context;
    uint8_t kind;
    bool valid;
} xhci_inventory_entry_t;
...
typedef struct xhci_state {
    ...
    xhci_inventory_entry_t inventory[XHCI_MAX_USB_INVENTORY];
...
static uint8_t xhci_current_device_kind(void);
...
static void xhci_copy_device_context(xhci_device_context_t *destination,
                                     const xhci_device_context_t *source) {
...
static void xhci_set_device_kind(uint8_t kind) {
    xhci_clear_device_flags();
    if (kind != 0U) g_xhci_usb_enumerated = true;
    if (kind == 1U) {
        g_xhci_usb_hid = true;
        g_xhci_usb_mouse = g_xhci.device.hid_protocol == USB_HID_PROTOCOL_MOUSE;
    } else if (kind == 2U) {
        ...
        g_xhci_usb_hub = true;
        g_xhci_usb_hub_ports = g_xhci.device.hub_port_count;
    }
}
...
static bool xhci_free_device_resources(xhci_state_t *state) {
    bool released = true;
    if (state == 0) return false;
    if (state->audio_stream != 0) {
        /* 热拔出先让统一音频对象进入 DISCONNECTED，再释放其端点 DMA。 */
        audio_stream_t *stream = state->audio_stream;
        state->audio_stream = 0;
        state->audio_stream_running = false;
        state->audio_stream_queued = false;
        state->audio_stream_bound = false;
        (void)audio_stream_disconnect(stream);
    }
    if (state->bt_controller != 0) {
        (void)bt_controller_disconnect(state->bt_controller);
        bt_controller_destroy(state->bt_controller);
        state->bt_controller = 0;
    }
    if (!xhci_free_page(&state->descriptor_buffer)) released = false;
    if (!xhci_free_page(&state->hid_report)) released = false;
    if (!xhci_free_page(&state->hid_ring)) released = false;
    if (!xhci_free_page(&state->audio_buffer)) released = false;
    if (!xhci_free_page(&state->audio_ring)) released = false;
    if (!xhci_free_page(&state->bt_acl_out_ring)) released = false;
    if (!xhci_free_page(&state->bt_acl_out_buffer)) released = false;
    if (!xhci_free_page(&state->bt_acl_in_buffer)) released = false;
    if (!xhci_free_page(&state->bt_acl_in_ring)) released = false;
    if (!xhci_free_page(&state->bt_event_buffer)) released = false;
    if (!xhci_free_page(&state->bt_event_ring)) released = false;
    if (!xhci_free_page(&state->hub_report)) released = false;
    if (!xhci_free_page(&state->hub_ring)) released = false;
    if (!xhci_free_page(&state->ep0_ring)) released = false;
    if (!xhci_free_page(&state->input_context)) released = false;
    if (!xhci_free_page(&state->output_context)) released = false;
    if (!released) return false;
    state->device_slot = 0;
    state->device_port = 0;
    state->device_speed = 0;
    state->root_port = 0U;
    state->parent_slot = 0U;
    state->parent_port = 0U;
    state->route_string = 0U;
    state->ep0_enqueue = 0;
    state->configuration_value = 0;
    state->hid_interface = 0U;
    state->hid_protocol = 0U;
    state->hid_endpoint = 0;
    state->hid_max_packet = 0;
    state->hid_interval = 0;
    state->hid_enqueue = 0;
    state->hid_cycle = 1U;
    state->hid_transfer_pending = false;
    state->hid_previous_modifier = 0U;
    for (uint32_t i = 0; i < 6U; ++i) state->hid_previous_keys[i] = 0U;
    state->hid_previous_buttons = 0U;
    state->audio_interface = 0U;
    state->audio_alt_setting = 0U;
    ...
    state->audio_endpoint_in = false;
    state->audio_enqueue = 0U;
    state->audio_cycle = 1U;
    state->audio_transfer_pending = false;
    state->audio_completed = 0U;
    state->audio_stream = 0;
    state->audio_period = 0U;
    state->audio_frames = 0U;
    state->audio_stream_running = false;
    state->audio_stream_queued = false;
    state->audio_stream_bound = false;
    ...
    state->bt_event_enqueue = 0U;
    state->bt_event_cycle = 1U;
    state->bt_event_transfer_pending = false;
    state->bt_acl_in_enqueue = 0U;
    state->bt_acl_in_cycle = 1U;
    state->bt_acl_in_transfer_pending = false;
    state->bt_acl_out_enqueue = 0U;
    state->bt_acl_out_cycle = 1U;
    state->bt_acl_out_transfer_pending = false;
    return true;
}
 ...
 * 后续仍会执行最小化的资源回收。
 */
static bool xhci_stop_pending_endpoints(xhci_state_t *state) {
    uint8_t endpoints[6];
    uint32_t count = 0U;
    bool stopped = true;
    if (state == 0 || state->device_slot == 0U) return true;
    if (state->hid_transfer_pending && state->hid_endpoint != 0U) {
        endpoints[count++] = (uint8_t)(state->hid_endpoint * 2U + 1U);
    }
    if (state->audio_transfer_pending && state->audio_endpoint != 0U) {
        endpoints[count++] = (uint8_t)(state->audio_endpoint * 2U +
                                       (state->audio_endpoint_in ? 1U : 0U));
    }
    if (state->hub_transfer_pending && state->hub_endpoint != 0U) {
        endpoints[count++] = (uint8_t)(state->hub_endpoint * 2U + 1U);
    }
    if (state->bt_event_transfer_pending && state->bt_event_endpoint != 0U) {
        endpoints[count++] = (uint8_t)(state->bt_event_endpoint * 2U + 1U);
    }
    if (state->bt_acl_in_transfer_pending && state->bt_acl_in_endpoint != 0U) {
        endpoints[count++] = (uint8_t)(state->bt_acl_in_endpoint * 2U + 1U);
    }
    if (state->bt_acl_out_transfer_pending && state->bt_acl_out_endpoint != 0U) {
        endpoints[count++] = (uint8_t)(state->bt_acl_out_endpoint * 2U);
    }
    for (uint32_t i = 0U; i < count; ++i) {
        ...
        if (!xhci_submit_command_ex(state, XHCI_STOP_ENDPOINT_TYPE,
                                    state->device_slot, endpoints[i], 0U, 0)) {
            stopped = false;
        }
    }
    return stopped;
...
static void xhci_recompute_topology(xhci_state_t *state) {
    bool hub_present = false;
    bool downstream = false;
    bool hid_present = false;
    bool mouse_present = false;
    uint8_t hub_ports = 0U;
    uint32_t count = 0U;
    if (state == 0) return;
    if (state->device.device_slot != 0U) {
        ++count;
        if (state->device.hid_endpoint != 0U) {
            hid_present = true;
            mouse_present = state->device.hid_protocol == USB_HID_PROTOCOL_MOUSE;
        }
        if (xhci_context_kind(&state->device) == 4U) {
            hub_present = true;
            hub_ports = state->device.hub_port_count;
        }
        if (state->device.parent_slot != 0U) downstream = true;
    }
    for (uint32_t i = 0U; i < state->inventory_count; ++i) {
        const xhci_inventory_entry_t *entry = &state->inventory[i];
        if (!entry->valid) continue;
        ++count;
        if (entry->context.hid_endpoint != 0U) {
            hid_present = true;
            if (entry->context.hid_protocol == USB_HID_PROTOCOL_MOUSE) {
                mouse_present = true;
            }
        }
        if (entry->kind == 4U) {
            hub_present = true;
            if (hub_ports == 0U) hub_ports = entry->context.hub_port_count;
        }
        if (entry->context.parent_slot != 0U) downstream = true;
    }
    g_xhci_usb_device_count = count;
    g_xhci_usb_enumerated = count != 0U;
    g_xhci_usb_hid = hid_present;
    g_xhci_usb_mouse = mouse_present;
    g_xhci_usb_hub_present = hub_present;
    g_xhci_usb_hub_inventory_ports = hub_ports;
    g_xhci_usb_hub_downstream = downstream;
}

static bool xhci_take_active_device(xhci_state_t *state,
                                    xhci_device_context_t *saved,
                                    uint8_t *kind) {
    if (state == 0 || saved == 0 || kind == 0) return false;
    xhci_zero_device_context(saved);
    if (state->device.device_slot == 0U) {
        *kind = 0U;
        xhci_clear_device_flags();
        return true;
    }
    *kind = xhci_current_device_kind();
    if (*kind == 0U) *kind = xhci_context_kind(&state->device);
    xhci_copy_device_context(saved, &state->device);
    xhci_zero_device_context(&state->device);
    xhci_clear_device_flags();
    return true;
}

static bool xhci_restore_active_device(xhci_state_t *state,
                                       xhci_device_context_t *saved,
                                       uint8_t kind) {
    if (state == 0 || saved == 0 || state->device.device_slot != 0U) return false;
    if (saved->device_slot == 0U) {
        xhci_clear_device_flags();
        return true;
    }
    xhci_copy_device_context(&state->device, saved);
    xhci_zero_device_context(saved);
    xhci_set_device_kind(kind != 0U ? kind : xhci_context_kind(&state->device));
    xhci_recompute_topology(state);
    return true;
}

static bool xhci_release_active_device(xhci_state_t *state) {
    bool released;
    if (state == 0 || state->device.device_slot == 0U) return true;
    if (!xhci_stop_pending_endpoints(state)) g_xhci_error = 60U;
    if (!xhci_submit_command(state, XHCI_DISABLE_SLOT_TYPE,
                             state->device_slot, 0U, 0)) {
        /* 设备已断开时 Disable Slot 可能没有完成事件，但 DMA 仍必须释放。 */
        g_xhci_error = 61U;
    }
    released = xhci_free_device_resources(state);
    xhci_clear_device_flags();
    xhci_recompute_topology(state);
    return released;
}

static bool xhci_release_inventory_entry(xhci_state_t *state, uint32_t index) {
    xhci_inventory_entry_t *entry;
    uint8_t kind;
    bool released;
    if (state == 0 || state->device.device_slot != 0U ||
        index >= state->inventory_count) return false;
    entry = &state->inventory[index];
    if (!entry->valid || entry->context.device_slot == 0U) return false;
    kind = entry->kind;
    xhci_copy_device_context(&state->device, &entry->context);
    xhci_zero_device_context(&entry->context);
    entry->valid = false;
    xhci_set_device_kind(kind);
    released = xhci_release_active_device(state);
    xhci_zero_device_context(&state->device);
    xhci_clear_device_flags();
    xhci_recompute_topology(state);
    return released;
...
static bool xhci_save_active_device(xhci_state_t *state, uint8_t kind) {
    xhci_inventory_entry_t *entry;
    if (state == 0 || state->device.device_slot == 0U ||
        state->inventory_count >= XHCI_MAX_USB_INVENTORY || kind == 0U) {
        return false;
    }
    entry = &state->inventory[state->inventory_count++];
    xhci_copy_device_context(&entry->context, &state->device);
    entry->kind = kind;
    entry->valid = true;
    if (kind == 4U) {
        g_xhci_usb_hub_present = true;
        g_xhci_usb_hub_inventory_ports = state->device.hub_port_count;
    }
    xhci_zero_device_context(&state->device);
    xhci_clear_device_flags();
    xhci_recompute_topology(state);
...
static bool xhci_load_inventory_device(xhci_state_t *state, uint32_t index) {
    xhci_inventory_entry_t *entry;
    if (state == 0 || state->device.device_slot != 0U ||
        index >= state->inventory_count) return false;
    entry = &state->inventory[index];
    ...
    xhci_zero_device_context(&entry->context);
    entry->valid = false;
    xhci_set_device_kind(entry->kind);
    xhci_recompute_topology(state);
    return true;
}

static bool xhci_free_inventory_resources(xhci_state_t *state) {
    bool released = true;
    if (state == 0) return false;
    for (uint32_t i = 0U; i < state->inventory_count; ++i) {
        xhci_inventory_entry_t *entry = &state->inventory[i];
        if (!entry->valid) continue;
        xhci_copy_device_context(&state->device, &entry->context);
        if (!xhci_stop_pending_endpoints(state)) released = false;
        if (!xhci_submit_command(state, XHCI_DISABLE_SLOT_TYPE,
                                 state->device_slot, 0U, 0)) released = false;
        if (!xhci_free_device_resources(state)) released = false;
        xhci_zero_device_context(&entry->context);
        entry->valid = false;
    }
    return released;
...
static bool xhci_queue_hub_status(xhci_state_t *state) {
    xhci_trb_t *ring;
    uint32_t index;
    uint32_t endpoint_id;
    if (state == 0 || !state->initialized || state->hub_ring.cpu == 0 ||
        state->hub_report.cpu == 0 || state->hub_endpoint == 0U ||
        state->hub_max_packet == 0U || state->hub_transfer_pending) return false;
    if (state->hub_max_packet > PAGE_SIZE) return false;
    index = state->hub_enqueue;
    if (index >= XHCI_RING_TRB_COUNT - 1U) return false;
    endpoint_id = (uint32_t)state->hub_endpoint * 2U + 1U;
    if (endpoint_id > 31U) return false;
    for (uint32_t i = 0U; i < state->hub_max_packet; ++i) {
        ((uint8_t *)state->hub_report.cpu)[i] = 0U;
    }
    ring = (xhci_trb_t *)state->hub_ring.cpu;
    ring[index].parameter = xhci_dma_address(&state->hub_report.mapping);
    ring[index].status = state->hub_max_packet;
    ring[index].control = (XHCI_NORMAL_TRB_TYPE << XHCI_TRB_TYPE_SHIFT) |
                          XHCI_TRB_INTERRUPT_ON_COMPLETION | state->hub_cycle;
    ++index;
    if (index == XHCI_RING_TRB_COUNT - 1U) {
        ring[XHCI_RING_TRB_COUNT - 1U].control =
            (XHCI_LINK_TRB_TYPE << XHCI_TRB_TYPE_SHIFT) |
            state->hub_cycle | XHCI_TRB_LINK_TOGGLE_CYCLE;
        state->hub_enqueue = 0U;
        state->hub_cycle ^= 1U;
    } else {
        state->hub_enqueue = index;
    }
    state->hub_transfer_pending = true;
    dma_sync_for_device(&state->hub_ring.mapping);
    dma_sync_for_device(&state->hub_report.mapping);
    dma_wmb();
    *(volatile uint32_t *)(state->mmio + state->doorbell_offset +
                           (uint32_t)state->device_slot * sizeof(uint32_t)) = endpoint_id;
    __asm__ volatile ("mfence" : : : "memory");
    return true;
}
...
static bool xhci_queue_audio_transfer(xhci_state_t *state) {
    xhci_trb_t *ring;
    uint32_t index;
    uint32_t endpoint_id;
    uint32_t transfer_bytes;
    if (state == 0 || !state->initialized || state->audio_ring.cpu == 0 ||
        state->audio_buffer.cpu == 0 || state->audio_transfer_pending ||
        state->audio_endpoint == 0U || state->audio_max_packet == 0U) return false;
    if (state->audio_stream != 0) {
        if (!state->audio_stream_queued || state->audio_frames == 0U) return false;
        uint64_t bytes = audio_stream_bytes_for_frames(state->audio_stream,
                                                       state->audio_frames);
        if (bytes == 0U || bytes > PAGE_SIZE) return false;
        transfer_bytes = (uint32_t)bytes;
        if (!state->audio_endpoint_in &&
            audio_stream_period_read(state->audio_stream, state->audio_period,
                                     state->audio_buffer.cpu, bytes) != K_OK) {
            return false;
        }
    } else {
        transfer_bytes = state->audio_max_packet;
    }
    index = state->audio_enqueue;
    if (index >= XHCI_RING_TRB_COUNT - 1U) return false;
    endpoint_id = (uint32_t)state->audio_endpoint * 2U +
                  (state->audio_endpoint_in ? 1U : 0U);
    ring = (xhci_trb_t *)state->audio_ring.cpu;
    ring[index].parameter = xhci_dma_address(&state->audio_buffer.mapping);
    ...
    ring[index].control = (XHCI_ISOCH_TRB_TYPE << XHCI_TRB_TYPE_SHIFT) |
                          ...
                          (state->audio_endpoint_in ? XHCI_TRB_DIRECTION_IN : 0U) |
                          state->audio_cycle;
    ++index;
    if (index == XHCI_RING_TRB_COUNT - 1U) {
        ring[XHCI_RING_TRB_COUNT - 1U].control =
            (XHCI_LINK_TRB_TYPE << XHCI_TRB_TYPE_SHIFT) |
            state->audio_cycle | XHCI_TRB_LINK_TOGGLE_CYCLE;
        state->audio_enqueue = 0U;
        state->audio_cycle ^= 1U;
    } else {
        state->audio_enqueue = index;
    }
    state->audio_transfer_pending = true;
...
static void xhci_detach_device(xhci_state_t *state) {
    uint8_t slot;
    if (state == 0 || state->device_slot == 0U) return;
    slot = state->device_slot;
    if (!xhci_stop_pending_endpoints(state)) g_xhci_error = 60U;
    if (!xhci_submit_command(state, XHCI_DISABLE_SLOT_TYPE, slot, 0U, 0)) {
        /* 设备已离线时，控制器可能不会再产生 Disable Slot 完成事件；
           无论如何都要先释放 DMA 资源，避免拔出后使用悬空地址。 */
        g_xhci_error = 61U;
    }
...
static bool xhci_inventory_has_root_port(const xhci_state_t *state,
                                         uint8_t port) {
    if (state == 0 || port == 0U) return false;
    if (state->device.device_slot != 0U && state->device.root_port == port) {
        return true;
    }
    for (uint32_t i = 0U; i < state->inventory_count; ++i) {
        const xhci_inventory_entry_t *entry = &state->inventory[i];
        if (entry->valid && entry->context.root_port == port) return true;
    }
    return false;
}

static bool xhci_activate_next_inventory_device(xhci_state_t *state) {
    if (state == 0 || state->device.device_slot != 0U) return false;
    /* 普通数据设备优先，Hub 只在没有其他活动设备时接管数据路径。 */
    for (uint32_t pass = 0U; pass < 2U; ++pass) {
        for (uint32_t i = 0U; i < state->inventory_count; ++i) {
            if (!state->inventory[i].valid ||
                (pass == 0U && state->inventory[i].kind == 4U)) continue;
            if (xhci_load_inventory_device(state, i)) {
                xhci_recompute_topology(state);
                return true;
            }
        }
...
static bool xhci_remove_root_port_devices(xhci_state_t *state, uint8_t port) {
    ...
    if (state->device.device_slot != 0U && state->device.root_port == port) {
        changed = true;
        (void)xhci_release_active_device(state);
    }
    for (uint32_t i = 0U; i < state->inventory_count; ++i) {
        if (!state->inventory[i].valid || state->inventory[i].context.root_port != port) {
            continue;
        }
        changed = true;
        (void)xhci_release_inventory_entry(state, i);
    }
    if (state->device.device_slot == 0U) (void)xhci_activate_next_inventory_device(state);
    xhci_recompute_topology(state);
    return changed;
}

static bool xhci_attach_root_runtime_device(xhci_state_t *state, uint8_t port) {
    xhci_device_context_t saved_active;
    uint8_t saved_kind = 0U;
    bool had_active;
    bool attached;
    uint8_t new_kind;
    if (state == 0 || port == 0U || port > state->max_ports) return false;
    had_active = state->device.device_slot != 0U;
    if (!xhci_take_active_device(state, &saved_active, &saved_kind)) return false;
    xhci_attach_device(state, port);
    attached = state->device.device_slot != 0U && g_xhci_usb_enumerated;
    if (!attached) {
        if (state->device.device_slot != 0U) (void)xhci_release_active_device(state);
        if (had_active) (void)xhci_restore_active_device(state, &saved_active, saved_kind);
        xhci_recompute_topology(state);
        return false;
    }
    new_kind = xhci_current_device_kind();
    if (had_active) {
        if (new_kind == 0U || !xhci_save_active_device(state, new_kind)) {
            (void)xhci_release_active_device(state);
            (void)xhci_restore_active_device(state, &saved_active, saved_kind);
            xhci_recompute_topology(state);
            return false;
        }
        (void)xhci_restore_active_device(state, &saved_active, saved_kind);
...
static bool xhci_handle_root_port_event(xhci_state_t *state, uint8_t port,
    ...
    } else if (!xhci_inventory_has_root_port(state, port)) {
...
static bool xhci_reenumerate_self_test(xhci_state_t *state) {
    uint8_t port;
    uint32_t offset;
    uint32_t portsc;
    if (state == 0 || state->device_slot == 0U || state->device_port == 0U ||
        state->parent_slot != 0U) {
        return true;
    }
    port = state->root_port != 0U ? state->root_port : state->device_port;
    offset = state->operational_offset + XHCI_PORT_REGISTER_BASE +
             (uint32_t)(port - 1U) * XHCI_PORT_REGISTER_STRIDE;

    /* 先走与真实拔出相同的 slot 禁用、端点停止和 DMA 释放路径。 */
    xhci_detach_device(state);
    portsc = xhci_read32(state, offset);
    ...
    xhci_attach_device(state, port);
    if (state->device_slot == 0U || !g_xhci_usb_enumerated) {
        if (g_xhci_error == 0U) g_xhci_error = 67U;
        return false;
    }
    return true;
}
...
static uint8_t xhci_current_device_kind(void) {
    return xhci_context_kind(&g_xhci.device);
}

static bool xhci_probe_connected_ports(xhci_state_t *state,
                                       uint32_t *active_index) {
    bool found = false;
    if (state == 0 || active_index == 0) return false;
    state->inventory_count = 0U;
    for (uint32_t i = 0U; i < XHCI_MAX_USB_INVENTORY; ++i) {
        state->inventory[i].valid = false;
        state->inventory[i].kind = 0U;
        xhci_zero_device_context(&state->inventory[i].context);
    }
    for (uint32_t port = 1U; port <= state->max_ports; ++port) {
        uint32_t offset = state->operational_offset + XHCI_PORT_REGISTER_BASE +
                          (port - 1U) * XHCI_PORT_REGISTER_STRIDE;
        uint32_t portsc = xhci_read32(state, offset);
        uint8_t speed = 0U;
        uint8_t slot = 0U;
        uint8_t kind;
        ...
        if (portsc == UINT32_MAX || (portsc & XHCI_PORTSC_CCS) == 0U ||
            ...
            !xhci_submit_command(state, XHCI_COMMAND_RING_TYPE, 0U, 0U, &slot) ||
            slot == 0U) {
            continue;
        }
        if (xhci_enumerate_device(state, slot, (uint8_t)port, speed,
            ...
            kind = xhci_current_device_kind();
            if (kind != 0U && xhci_save_active_device(state, kind)) {
                ++g_xhci_usb_device_count;
                continue;
            }
            if (kind != 0U) save_failed = true;
        }
        if (slot != 0U) (void)xhci_submit_command(state, XHCI_DISABLE_SLOT_TYPE, slot, 0U, 0);
    ...
    if (!xhci_probe_hub_downstream(state)) return false;
    /* 优先把普通功能设备保留为活动设备，Hub 仅在没有其他可用设备
     * 时成为活动设备，避免改变现有输入/音频路径。 */
    for (uint32_t pass = 0U; pass < 2U && !found; ++pass) {
        for (uint32_t i = 0U; i < state->inventory_count; ++i) {
            const xhci_inventory_entry_t *entry = &state->inventory[i];
            if (!entry->valid || (pass == 0U && entry->kind == 4U)) continue;
            *active_index = i;
            found = true;
            break;
        }
...
static bool xhci_handle_hub_transfer_event(xhci_state_t *state,
                                           const xhci_trb_t *event) {
    xhci_inventory_entry_t *entry = 0;
    xhci_device_context_t saved_active;
    uint8_t slot;
    uint32_t endpoint_id;
    uint32_t completion;
    ...
    active = state->device_slot == slot && state->hub_endpoint != 0U;
    if (!active) {
        for (uint32_t i = 0U; i < state->inventory_count; ++i) {
            if (state->inventory[i].valid &&
                state->inventory[i].context.device_slot == slot &&
                state->inventory[i].context.hub_endpoint != 0U) {
                entry = &state->inventory[i];
                break;
            }
        }
    }
    if ((!active && entry == 0) || slot == 0U) return false;
    if (endpoint_id != (uint32_t)(active ? state->hub_endpoint :
                                  entry->context.hub_endpoint) * 2U + 1U) {
        return false;
    }
    if (!active) {
        xhci_copy_device_context(&saved_active, &state->device);
        xhci_copy_device_context(&state->device, &entry->context);
    }
    completion = event->status >> XHCI_COMPLETION_SHIFT;
    state->hub_transfer_pending = false;
    if (completion == XHCI_COMPLETION_SUCCESS) {
        uint32_t bytes = (state->hub_port_count + 8U) / 8U;
        uint32_t bitmap = 0U;
        if (bytes > state->hub_max_packet) bytes = state->hub_max_packet;
        if (bytes > PAGE_SIZE) bytes = PAGE_SIZE;
        dma_sync_for_cpu(&state->hub_report.mapping);
        for (uint32_t port = 1U; port <= state->hub_port_count; ++port) {
            uint32_t byte = port / 8U;
            uint32_t bit = port % 8U;
            if (byte < bytes &&
                (((const uint8_t *)state->hub_report.cpu)[byte] & (1U << bit)) != 0U) {
                bitmap |= 1U << (port - 1U);
            }
        }
        state->hub_change_bitmap |= bitmap;
        (void)xhci_queue_hub_status(state);
    } else if (completion != 0U) {
        /* 设备拔出时端点可能返回停止/取消；拓扑轮询仍负责最终清理。 */
        g_xhci_error = 73U + completion;
    }
    if (!active) {
        xhci_copy_device_context(&entry->context, &state->device);
        xhci_zero_device_context(&state->device);
        xhci_copy_device_context(&state->device, &saved_active);
    }
    return true;
}

static xhci_device_context_t *xhci_find_hid_context(xhci_state_t *state,
                                                     uint8_t slot,
                                                     uint32_t endpoint_id) {
    xhci_device_context_t *device;
    if (state == 0 || slot == 0U) return 0;
    device = &state->device;
    if (device->device_slot == slot && device->hid_endpoint != 0U &&
        endpoint_id == (uint32_t)device->hid_endpoint * 2U + 1U) {
        return device;
    }
    for (uint32_t i = 0U; i < state->inventory_count; ++i) {
        xhci_inventory_entry_t *entry = &state->inventory[i];
        device = &entry->context;
        if (entry->valid && device->device_slot == slot &&
...
static bool xhci_process_event_ring(xhci_state_t *state, uint32_t budget) {
    ...
    for (uint32_t i = 0U; i < budget; ++i) {
        ...
        if (type == XHCI_PORT_STATUS_CHANGE_TYPE) {
            ...
            consumed = true;
        } else if (type == XHCI_TRANSFER_EVENT_TYPE && state->audio_endpoint != 0U &&
                   (uint8_t)(event.control >> XHCI_TRB_SLOT_SHIFT) == state->device_slot &&
                   ((event.control >> XHCI_TRB_ENDPOINT_SHIFT) & 0x1FU) ==
                       (uint32_t)state->audio_endpoint * 2U +
                           (state->audio_endpoint_in ? 1U : 0U)) {
            uint32_t completion = event.status >> XHCI_COMPLETION_SHIFT;
            state->audio_transfer_pending = false;
            if (completion == XHCI_COMPLETION_SUCCESS) {
                dma_sync_for_cpu(&state->audio_buffer.mapping);
                ++state->audio_completed;
                if (state->audio_stream != 0) {
                    if (state->audio_endpoint_in && state->audio_stream_queued) {
                        uint64_t bytes = audio_stream_bytes_for_frames(
                            state->audio_stream, state->audio_frames);
                        if (bytes != 0U && bytes <= PAGE_SIZE) {
                            (void)audio_stream_period_write(
                                state->audio_stream, state->audio_period,
                                state->audio_buffer.cpu, bytes);
                        }
                    }
                    /* COMPLETE 由音频 UAPI 的拥有者确认；这里仅释放后端
                       的硬件占用，避免内核与用户同时消费同一周期。 */
                    state->audio_stream_queued = false;
                } else if (!state->audio_stream_bound) {
                    (void)xhci_queue_audio_transfer(state);
                }
                consumed = true;
            } else if (completion != 0U) {
                state->audio_stream_queued = false;
                g_xhci_error = 64U + completion;
            }
        }
    }
...
static bool xhci_prepare_device_resources(xhci_state_t *state, uint8_t slot,
    ...
    control[0] = 0U;
    control[1] = 0x3U;
    slot_context[0] = ((uint32_t)(speed & XHCI_PORTSC_SPEED_MASK) << 20) |
                      (route_string & 0x000FFFFFU) | (1U << 27);
    slot_context[1] = (uint32_t)root_port << 16;
    slot_context[2] = (uint32_t)parent_slot | ((uint32_t)parent_port << 8);
    slot_context[3] = 0U;
...
static bool xhci_submit_control_transfer(xhci_state_t *state,
                                         const uint8_t setup[8],
                                         uint32_t length, bool direction_in) {
    if (state == 0 || setup == 0 || length > PAGE_SIZE ||
        state->device_slot == 0U) return false;
    xhci_trb_t *ring = (xhci_trb_t *)state->ep0_ring.cpu;
    uint32_t index = state->ep0_enqueue;
    ...
    if (length != 0U) {
        ring[index + 1U].parameter =
            xhci_dma_address(&state->descriptor_buffer.mapping);
    ...
    state->ep0_enqueue = index + transfer_trbs;
    dma_sync_for_device(&state->ep0_ring.mapping);
    dma_sync_for_device(&state->descriptor_buffer.mapping);
    dma_wmb();
    *(volatile uint32_t *)(state->mmio + state->doorbell_offset +
                           (uint32_t)state->device_slot * sizeof(uint32_t)) = 1U;
    __asm__ volatile ("mfence" : : : "memory");
    for (uint32_t spin = 0; spin < 200000U; ++spin) {
        xhci_trb_t event;
        if (!xhci_next_ring_event(state, &event)) {
        ...
        if ((uint8_t)(event.control >> XHCI_TRB_SLOT_SHIFT) != state->device_slot ||
        ...
        dma_sync_for_cpu(&state->descriptor_buffer.mapping);
        return true;
    }
    g_xhci_error = 54U;
    return false;
...
static bool xhci_read_hub_descriptor(xhci_state_t *state) {
    ...
    if (state == 0 || !state->hub_interface_present ||
        !xhci_submit_control_transfer(state, request, 9U, true)) return false;
    ...
    state->hub_port_count = descriptor[2];
    ...
    for (uint32_t offset = 0U; offset + 2U <= total_length;) {
        ...
        if (type == XHCI_DESCRIPTOR_INTERFACE && length >= 9U) {
            ...
            audio_interface = interface_class == USB_CLASS_AUDIO &&
                              ...
                              alternate != 0U;
            hub_interface = interface_class == USB_CLASS_HUB;
...
static bool xhci_hub_get_port_status(xhci_state_t *state, uint8_t port,
                                     uint32_t *status_out) {
    uint8_t request[8] = {
        0xA3U, XHCI_SETUP_GET_STATUS, 0x00U, 0x00U,
        port, 0x00U, 0x04U, 0x00U,
    };
    uint8_t *status;
    if (state == 0 || status_out == 0 || port == 0U ||
        !xhci_submit_control_transfer(state, request, 4U, true)) return false;
    status = (uint8_t *)state->descriptor_buffer.cpu;
    *status_out = (uint32_t)status[0] | ((uint32_t)status[1] << 8) |
                  ((uint32_t)status[2] << 16) | ((uint32_t)status[3] << 24);
    return true;
}

static bool xhci_hub_set_port_feature(xhci_state_t *state, uint8_t port,
                                      uint16_t feature) {
    uint8_t request[8] = {
        0x23U, XHCI_SETUP_SET_FEATURE, (uint8_t)feature,
        (uint8_t)(feature >> 8), port, 0x00U, 0x00U, 0x00U,
    };
    return state != 0 && port != 0U &&
           xhci_submit_control_transfer(state, request, 0U, false);
}
...
static bool xhci_probe_hub_downstream(xhci_state_t *state) {
    for (uint32_t hub_index = 0U; hub_index < state->inventory_count; ++hub_index) {
        xhci_inventory_entry_t *entry = &state->inventory[hub_index];
        xhci_device_context_t hub_context;
        if (!entry->valid || entry->kind != 4U) continue;
        xhci_copy_device_context(&hub_context, &entry->context);
        xhci_zero_device_context(&entry->context);
        entry->valid = false;
        for (uint32_t port = 1U; port <= hub_context.hub_port_count; ++port) {
            uint32_t status = 0U;
            uint8_t speed;
            uint32_t route = 0U;
            uint8_t slot = 0U;
            bool enabled = false;
            xhci_copy_device_context(&state->device, &hub_context);
            xhci_set_device_kind(4U);
            if (!xhci_hub_get_port_status(state, (uint8_t)port, &status) ||
                (status & 1U) == 0U) {
                xhci_zero_device_context(&state->device);
                continue;
            }
            (void)xhci_hub_set_port_feature(state, (uint8_t)port,
                                            USB_HUB_FEATURE_PORT_POWER);
            (void)xhci_hub_set_port_feature(state, (uint8_t)port,
                                            USB_HUB_FEATURE_PORT_RESET);
            for (uint32_t spin = 0U; spin < 10000U; ++spin) {
                if (xhci_hub_get_port_status(state, (uint8_t)port, &status) &&
                    (status & 1U) != 0U && (status & 2U) != 0U) {
                    enabled = true;
                    break;
                }
                __asm__ volatile ("pause");
            }
            if (!enabled || !xhci_child_route(&hub_context, (uint8_t)port, &route)) {
                xhci_zero_device_context(&state->device);
                continue;
            }
            speed = xhci_hub_port_speed(status);
            /* Hub 的 DMA 页面由 hub_context 持有；子设备必须从空的
             * 活动 context 开始分配，不能覆盖父 Hub 的页面句柄。 */
            xhci_zero_device_context(&state->device);
            if (!xhci_submit_command(state, XHCI_COMMAND_RING_TYPE, 0U, 0U, &slot) ||
                slot == 0U || !xhci_enumerate_device(
                    state, slot, (uint8_t)port, speed, hub_context.root_port,
                    hub_context.device_slot, (uint8_t)port, route)) {
                if (slot != 0U) {
                    (void)xhci_submit_command(state, XHCI_DISABLE_SLOT_TYPE,
                                               slot, 0U, 0);
                }
                (void)xhci_free_device_resources(state);
                xhci_clear_device_flags();
            ...
            uint8_t kind = xhci_current_device_kind();
            if (kind != 0U && xhci_save_active_device(state, kind)) {
                ++g_xhci_usb_device_count;
                g_xhci_usb_hub_downstream = true;
            } else {
                (void)xhci_submit_command(state, XHCI_DISABLE_SLOT_TYPE,
                                           slot, 0U, 0);
                (void)xhci_free_device_resources(state);
        ...
        xhci_copy_device_context(&entry->context, &hub_context);
        entry->valid = true;
        xhci_zero_device_context(&state->device);
        xhci_clear_device_flags();
    }
...
static bool xhci_remove_device_subtree(xhci_state_t *state,
                                       xhci_device_context_t *saved_active,
                                       uint8_t *saved_kind, uint8_t slot) {
    bool removed = false;
    if (state == 0 || saved_active == 0 || saved_kind == 0 || slot == 0U) {
        return false;
    }
    if (saved_active->device_slot == slot) {
        xhci_copy_device_context(&state->device, saved_active);
        xhci_zero_device_context(saved_active);
        xhci_set_device_kind(*saved_kind);
        (void)xhci_release_active_device(state);
        xhci_zero_device_context(&state->device);
        xhci_clear_device_flags();
        *saved_kind = 0U;
    ...
    for (uint32_t i = 0U; i < state->inventory_count; ++i) {
        if (!state->inventory[i].valid ||
            state->inventory[i].context.device_slot != slot) continue;
        (void)xhci_release_inventory_entry(state, i);
        removed = true;
        break;
    }
    /* 先移除直接子节点，再递归清理更深层 Hub 下游。 */
    for (uint32_t i = 0U; i < state->inventory_count; ++i) {
        if (!state->inventory[i].valid ||
            state->inventory[i].context.parent_slot != slot) continue;
        uint8_t child_slot = state->inventory[i].context.device_slot;
        if (xhci_remove_device_subtree(state, saved_active, saved_kind,
                                       child_slot)) removed = true;
    }
    if (saved_active->device_slot != 0U && saved_active->parent_slot == slot) {
        uint8_t child_slot = saved_active->device_slot;
        if (xhci_remove_device_subtree(state, saved_active, saved_kind,
                                       child_slot)) removed = true;
    }
    return removed;
...
static bool xhci_find_hub_child(const xhci_state_t *state,
                                const xhci_device_context_t *saved_active,
                                uint8_t hub_slot, uint8_t port,
                                uint32_t *inventory_index, bool *active_child) {
    if (state == 0 || saved_active == 0 || inventory_index == 0 ||
        active_child == 0) return false;
    *inventory_index = UINT32_MAX;
    *active_child = saved_active->device_slot != 0U &&
                    saved_active->parent_slot == hub_slot &&
                    saved_active->parent_port == port;
    for (uint32_t i = 0U; i < state->inventory_count; ++i) {
        const xhci_inventory_entry_t *entry = &state->inventory[i];
        if (entry->valid && entry->context.parent_slot == hub_slot &&
            entry->context.parent_port == port) {
            *inventory_index = i;
            return true;
        }
    }
...
static bool xhci_reconcile_one_hub(xhci_state_t *state,
                              const xhci_device_context_t *hub,
                              xhci_device_context_t *saved_active,
                              uint8_t *saved_kind) {
    bool changed = false;
    if (state == 0 || hub == 0 || saved_active == 0 || saved_kind == 0 ||
        hub->device_slot == 0U || hub->hub_port_count == 0U) return false;
    for (uint32_t port = 1U; port <= hub->hub_port_count; ++port) {
        uint32_t status = 0U;
        uint32_t child_index = UINT32_MAX;
        bool active_child = false;
        bool connected;
        xhci_copy_device_context(&state->device, hub);
        xhci_set_device_kind(4U);
        if (!xhci_hub_get_port_status(state, (uint8_t)port, &status)) {
            xhci_zero_device_context(&state->device);
            xhci_clear_device_flags();
        ...
        (void)xhci_find_hub_child(state, saved_active, hub->device_slot,
                                  (uint8_t)port, &child_index, &active_child);
        if (!connected) {
            uint8_t child_slot = 0U;
            if (active_child) child_slot = saved_active->device_slot;
            else if (child_index != UINT32_MAX) {
                child_slot = state->inventory[child_index].context.device_slot;
            }
            xhci_zero_device_context(&state->device);
            xhci_clear_device_flags();
            if (child_slot != 0U &&
                xhci_remove_device_subtree(state, saved_active, saved_kind,
                                            child_slot)) {
                changed = true;
            }
            continue;
        }
        if (active_child || child_index != UINT32_MAX) {
            xhci_zero_device_context(&state->device);
            xhci_clear_device_flags();
            continue;
        }

        /* 新连接设备需要由 Hub 供电并复位，随后才能取得可用速度。 */
        (void)xhci_hub_set_port_feature(state, (uint8_t)port,
                                        USB_HUB_FEATURE_PORT_POWER);
        (void)xhci_hub_set_port_feature(state, (uint8_t)port,
                                        USB_HUB_FEATURE_PORT_RESET);
        ...
        uint32_t route = 0U;
        if (!enabled || !xhci_child_route(hub, (uint8_t)port, &route)) {
            xhci_zero_device_context(&state->device);
            xhci_clear_device_flags();
            continue;
        }
        ...
        uint8_t slot = 0U;
        /* Hub context 的 DMA 所有权由局部副本持有，子设备从空 context 分配。 */
        xhci_zero_device_context(&state->device);
        if (!xhci_submit_command(state, XHCI_COMMAND_RING_TYPE, 0U, 0U, &slot) ||
            slot == 0U || !xhci_enumerate_device(
                state, slot, (uint8_t)port, speed, hub->root_port,
                hub->device_slot, (uint8_t)port, route)) {
            if (slot != 0U) {
                (void)xhci_submit_command(state, XHCI_DISABLE_SLOT_TYPE,
                                           slot, 0U, 0);
            }
...
static bool xhci_reconcile_hub_topology(xhci_state_t *state) {
    ...
    uint8_t saved_kind = 0U;
    bool changed = false;
    if (state == 0 || !state->initialized || !g_xhci_usb_hub_present) return false;
    if (!xhci_take_active_device(state, &saved_active, &saved_kind)) return false;
    if (saved_kind == 4U && saved_active.device_slot != 0U) {
        changed |= xhci_reconcile_one_hub(state, &saved_active, &saved_active,
                                     &saved_kind);
    }
    for (uint32_t i = 0U; i < state->inventory_count; ++i) {
        if (!state->inventory[i].valid || state->inventory[i].kind != 4U) continue;
        xhci_device_context_t hub;
        xhci_copy_device_context(&hub, &state->inventory[i].context);
        changed |= xhci_reconcile_one_hub(state, &hub, &saved_active, &saved_kind);
    }
    if (saved_active.device_slot != 0U) {
        (void)xhci_restore_active_device(state, &saved_active, saved_kind);
    } else if (state->device.device_slot == 0U) {
        (void)xhci_activate_next_inventory_device(state);
    }
    xhci_recompute_topology(state);
    if (changed) liteos_serial_write("LITEOS_USB_RUNTIME_HUB_CHANGE\r\n");
    return changed;
}
...
static bool xhci_queue_bt_event(xhci_state_t *state) {
    xhci_trb_t *ring;
    uint32_t endpoint_id;
    uint32_t index;
    if (state == 0 || state->bt_controller == 0 || state->bt_event_endpoint == 0U ||
        state->bt_event_ring.cpu == 0 || state->bt_event_buffer.cpu == 0 ||
        state->bt_event_transfer_pending) return false;
    endpoint_id = (uint32_t)state->bt_event_endpoint * 2U + 1U;
    index = state->bt_event_enqueue;
    if (endpoint_id > 31U || index >= XHCI_RING_TRB_COUNT - 1U ||
        state->bt_event_max_packet == 0U) return false;
    ring = (xhci_trb_t *)state->bt_event_ring.cpu;
    ring[index].parameter = xhci_dma_address(&state->bt_event_buffer.mapping);
    ring[index].status = state->bt_event_max_packet;
    ring[index].control = (XHCI_NORMAL_TRB_TYPE << XHCI_TRB_TYPE_SHIFT) |
                          XHCI_TRB_DIRECTION_IN |
                          XHCI_TRB_INTERRUPT_ON_COMPLETION | state->bt_event_cycle;
    ++index;
    if (index == XHCI_RING_TRB_COUNT - 1U) {
        ring[XHCI_RING_TRB_COUNT - 1U].control =
            (XHCI_LINK_TRB_TYPE << XHCI_TRB_TYPE_SHIFT) |
            state->bt_event_cycle | XHCI_TRB_LINK_TOGGLE_CYCLE;
        state->bt_event_enqueue = 0U;
        state->bt_event_cycle ^= 1U;
    } else {
        state->bt_event_enqueue = index;
    }
    state->bt_event_transfer_pending = true;
    dma_sync_for_device(&state->bt_event_ring.mapping);
    dma_sync_for_device(&state->bt_event_buffer.mapping);
    dma_wmb();
    *(volatile uint32_t *)(state->mmio + state->doorbell_offset +
                           (uint32_t)state->device_slot * sizeof(uint32_t)) = endpoint_id;
    __asm__ volatile ("mfence" : : : "memory");
...
static bool xhci_queue_bt_acl_in(xhci_state_t *state) {
    xhci_trb_t *ring;
    uint32_t endpoint_id;
    uint32_t index;
    if (state == 0 || state->bt_controller == 0 || state->bt_acl_in_endpoint == 0U ||
        state->bt_acl_in_ring.cpu == 0 || state->bt_acl_in_buffer.cpu == 0 ||
        state->bt_acl_in_transfer_pending) return false;
    endpoint_id = (uint32_t)state->bt_acl_in_endpoint * 2U + 1U;
    index = state->bt_acl_in_enqueue;
    if (endpoint_id > 31U || index >= XHCI_RING_TRB_COUNT - 1U) return false;
    ring = (xhci_trb_t *)state->bt_acl_in_ring.cpu;
    ring[index].parameter = xhci_dma_address(&state->bt_acl_in_buffer.mapping);
    ring[index].status = PAGE_SIZE;
    ring[index].control = (XHCI_NORMAL_TRB_TYPE << XHCI_TRB_TYPE_SHIFT) |
                          XHCI_TRB_DIRECTION_IN |
                          XHCI_TRB_INTERRUPT_ON_COMPLETION | state->bt_acl_in_cycle;
    ++index;
    if (index == XHCI_RING_TRB_COUNT - 1U) {
        ring[XHCI_RING_TRB_COUNT - 1U].control =
            (XHCI_LINK_TRB_TYPE << XHCI_TRB_TYPE_SHIFT) |
            state->bt_acl_in_cycle | XHCI_TRB_LINK_TOGGLE_CYCLE;
        state->bt_acl_in_enqueue = 0U;
        state->bt_acl_in_cycle ^= 1U;
    } else {
        state->bt_acl_in_enqueue = index;
    }
    state->bt_acl_in_transfer_pending = true;
    dma_sync_for_device(&state->bt_acl_in_ring.mapping);
    dma_sync_for_device(&state->bt_acl_in_buffer.mapping);
    dma_wmb();
    *(volatile uint32_t *)(state->mmio + state->doorbell_offset +
                           (uint32_t)state->device_slot * sizeof(uint32_t)) = endpoint_id;
    __asm__ volatile ("mfence" : : : "memory");
...
static bool xhci_queue_bt_acl_out(xhci_state_t *state, size_t length) {
    xhci_trb_t *ring;
    uint32_t endpoint_id;
    uint32_t index;
    if (state == 0 || state->bt_acl_out_endpoint == 0U ||
        state->bt_acl_out_ring.cpu == 0 || state->bt_acl_out_buffer.cpu == 0 ||
        state->bt_acl_out_buffer.mapping.device == 0 || length == 0U ||
        length > PAGE_SIZE || state->bt_acl_out_buffer.mapping.segment_count == 0U) {
        return false;
    }
    endpoint_id = (uint32_t)state->bt_acl_out_endpoint * 2U;
    index = state->bt_acl_out_enqueue;
    if (endpoint_id > 31U || index >= XHCI_RING_TRB_COUNT - 1U) return false;
    ring = (xhci_trb_t *)state->bt_acl_out_ring.cpu;
    ring[index].parameter = xhci_dma_address(&state->bt_acl_out_buffer.mapping);
    ring[index].status = (uint32_t)length;
    ring[index].control = (XHCI_NORMAL_TRB_TYPE << XHCI_TRB_TYPE_SHIFT) |
                          XHCI_TRB_INTERRUPT_ON_COMPLETION | state->bt_acl_out_cycle;
    ++index;
    if (index == XHCI_RING_TRB_COUNT - 1U) {
        ring[XHCI_RING_TRB_COUNT - 1U].control =
            (XHCI_LINK_TRB_TYPE << XHCI_TRB_TYPE_SHIFT) |
            state->bt_acl_out_cycle | XHCI_TRB_LINK_TOGGLE_CYCLE;
        state->bt_acl_out_enqueue = 0U;
        state->bt_acl_out_cycle ^= 1U;
    } else {
        state->bt_acl_out_enqueue = index;
    }
    state->bt_acl_out_transfer_pending = true;
    dma_sync_for_device(&state->bt_acl_out_ring.mapping);
    dma_sync_for_device(&state->bt_acl_out_buffer.mapping);
    dma_wmb();
    *(volatile uint32_t *)(state->mmio + state->doorbell_offset +
                           (uint32_t)state->device_slot * sizeof(uint32_t)) = endpoint_id;
    __asm__ volatile ("mfence" : : : "memory");
...
static kstatus_t xhci_bt_send(void *context, const uint8_t *packet, size_t length) {
    xhci_state_t *state = (xhci_state_t *)context;
    uint8_t setup[8] = {0x20U, 0x00U, 0x00U, 0x00U, 0x00U, 0x00U, 0x00U, 0x00U};
    if (state == 0 || packet == 0 || length < 4U || length - 1U > PAGE_SIZE) {
        return K_EINVAL;
    }
    if (packet[0] == 0x02U) {
        if (state->bt_acl_out_endpoint == 0U || state->bt_acl_out_transfer_pending ||
            state->bt_acl_out_buffer.cpu == 0 || length - 1U > PAGE_SIZE) return K_EBUSY;
        for (size_t i = 1U; i < length; ++i) {
            ((uint8_t *)state->bt_acl_out_buffer.cpu)[i - 1U] = packet[i];
        }
        return xhci_queue_bt_acl_out(state, length - 1U) ? K_OK : K_EIO;
    }
    if (packet[0] != 0x01U || state->descriptor_buffer.cpu == 0) return K_EINVAL;
    /* USB Bluetooth 的 control pipe 传输 opcode/length/parameters，不含 HCI 类型字节。 */
    for (size_t i = 1U; i < length; ++i) ((uint8_t *)state->descriptor_buffer.cpu)[i - 1U] = packet[i];
    setup[6] = (uint8_t)((length - 1U) & 0xFFU);
    setup[7] = (uint8_t)((length - 1U) >> 8);
    dma_sync_for_device(&state->descriptor_buffer.mapping);
    return xhci_submit_control_transfer(state, setup, (uint32_t)(length - 1U), false) ?
           K_OK : K_EIO;
}
...
static bool xhci_configure_bt_endpoints(xhci_state_t *state) {
    ...
    if (!xhci_submit_command(state, XHCI_CONFIGURE_ENDPOINT_TYPE,
                             state->device_slot,
                             xhci_dma_address(&state->input_context.mapping), 0) ||
        bt_controller_create(xhci_bt_send, state, &state->bt_controller) != K_OK ||
        bt_controller_start(state->bt_controller) != K_OK ||
        !xhci_queue_bt_event(state) ||
        (acl_in_id != 0U && !xhci_queue_bt_acl_in(state))) return false;
    return true;
}
...
static bool xhci_handle_bt_transfer_event(xhci_state_t *state,
    ...
    uint32_t residual;
    size_t length;
    if (state == 0 || event == 0 || state->bt_controller == 0 ||
        (uint8_t)(event->control >> XHCI_TRB_SLOT_SHIFT) != state->device_slot ||
        (event->control >> XHCI_TRB_TYPE_SHIFT & 0x3FU) != XHCI_TRANSFER_EVENT_TYPE) return false;
    endpoint_id = (event->control >> XHCI_TRB_ENDPOINT_SHIFT) & 0x1FU;
    completion = event->status >> XHCI_COMPLETION_SHIFT;
    residual = event->status & 0x00FFFFFFU;
    if (endpoint_id == (uint32_t)state->bt_event_endpoint * 2U + 1U) {
        state->bt_event_transfer_pending = false;
        if (completion == XHCI_COMPLETION_SUCCESS && residual <= state->bt_event_max_packet) {
            length = state->bt_event_max_packet - residual;
            dma_sync_for_cpu(&state->bt_event_buffer.mapping);
            if (length != 0U) (void)bt_hci_receive_event(state->bt_controller,
                state->bt_event_buffer.cpu, length);
            (void)xhci_queue_bt_event(state);
        } else {
            g_xhci_error = 70U + completion;
        }
        return true;
    }
    if (state->bt_acl_in_endpoint != 0U &&
        endpoint_id == (uint32_t)state->bt_acl_in_endpoint * 2U + 1U) {
        state->bt_acl_in_transfer_pending = false;
        if (completion == XHCI_COMPLETION_SUCCESS && residual <= PAGE_SIZE) {
            length = PAGE_SIZE - residual;
            dma_sync_for_cpu(&state->bt_acl_in_buffer.mapping);
            if (length != 0U) (void)bt_acl_receive(state->bt_controller,
                state->bt_acl_in_buffer.cpu, length);
            (void)xhci_queue_bt_acl_in(state);
        } else {
            g_xhci_error = 71U + completion;
        }
        return true;
    }
    if (state->bt_acl_out_endpoint != 0U &&
        endpoint_id == (uint32_t)state->bt_acl_out_endpoint * 2U) {
        state->bt_acl_out_transfer_pending = false;
        if (completion != XHCI_COMPLETION_SUCCESS) g_xhci_error = 72U + completion;
        return true;
    }
    return false;
}
...
static bool xhci_configure_hid_endpoint(xhci_state_t *state) {
    ...
    control[1] = 1U | (1U << endpoint_id);
    endpoint[0] = ((uint32_t)state->hid_interval << 16);
...
static bool xhci_enumerate_device(xhci_state_t *state, uint8_t slot,
    ...
    if (!xhci_submit_control_transfer(state, get_device_descriptor,
    ...
    if (!xhci_submit_control_transfer(state, get_configuration_descriptor,
                                      0xFFU, true) ||
    ...
    if (!xhci_submit_control_transfer(state, set_configuration, 0U, false)) {
        g_xhci_error = 58U;
        return false;
    }
    ...
    if (state->hub_interface_present) {
        if (!xhci_read_hub_descriptor(state)) return false;
        ...
        if (!xhci_submit_control_transfer(state, set_interface, 0U, false)) {
            g_xhci_error = 59U;
            return false;
        }
    }
    if (state->hid_endpoint != 0U) {
        ...
        if (!xhci_submit_control_transfer(state, set_protocol, 0U, false)) {
            g_xhci_error = 59U;
            return false;
        }
...
static bool xhci_destroy(xhci_state_t *state) {
    if (state == 0) return false;
    g_xhci_runtime_ready = false;
    g_xhci_runtime_event_batches = 0U;
    xhci_unbind_msix(state);
    if (state->mmio != 0) {
        (void)xhci_write32(state, state->operational_offset + XHCI_USBCMD, 0U);
        for (uint32_t spin = 0; spin < 10000U; ++spin) {
            if ((xhci_read32(state, state->operational_offset + XHCI_USBSTS) &
                 XHCI_USBSTS_HCH) != 0) break;
            __asm__ volatile ("pause");
        }
    }
    if (!xhci_free_device_resources(state) ||
        !xhci_free_inventory_resources(state) ||
        !xhci_free_rings(state)) return false;
    xhci_zero_device_context(&state->device);
    state->inventory_count = 0U;
    xhci_clear_device_flags();
    xhci_unmap_mmio(state);
...
bool xhci_hardware_self_test(void) {
    ...
    bool has_device = xhci_probe_connected_ports(&g_xhci, &active_index);
    ...
    bool success;
    if (has_device) {
        success = xhci_load_inventory_device(&g_xhci, active_index);
        if (!success) g_xhci_error = 69U;
    } else {
    ...
    if (success && has_device) {
        if (!xhci_reenumerate_self_test(&g_xhci)) {
...
uint32_t xhci_usb_audio_completed(void) {
    return g_xhci.audio_completed;
}

struct device *xhci_audio_device(void) {
    return g_xhci.initialized && g_xhci_usb_audio && g_xhci.pci != 0 ?
           (struct device *)&g_xhci.pci->device : 0;
}

static void xhci_audio_lock(void) {
...
static void xhci_audio_unlock(void) {
    atomic_store_explicit(&g_xhci.event_lock.state, 0U, memory_order_release);
}

static bool xhci_audio_format_supported(const audio_format_t *format) {
    uint32_t sample_bits;
    if (format == 0 || format->channels != g_xhci.audio_channels ||
        format->sample_rate != g_xhci.audio_sample_rate) return false;
    switch (format->sample_format) {
        case AUDIO_SAMPLE_S16_LE: sample_bits = 16U; break;
        case AUDIO_SAMPLE_S24_LE: sample_bits = 24U; break;
        case AUDIO_SAMPLE_S32_LE: sample_bits = 32U; break;
        default: return false;
    }
    return sample_bits == g_xhci.audio_bit_resolution;
}

kstatus_t xhci_audio_stream_configure(struct audio_stream *stream) {
    audio_stream_t *audio = (audio_stream_t *)stream;
    audio_format_t format;
    if (audio == 0 || !g_xhci.initialized || !g_xhci_usb_audio ||
        audio_stream_device(audio) != xhci_audio_device()) return K_ENOENT;
    audio_stream_get_format(audio, &format);
    if (audio_stream_direction(audio) !=
            (g_xhci.audio_endpoint_in ? AUDIO_CAPTURE : AUDIO_PLAYBACK) ||
        !xhci_audio_format_supported(&format) ||
        audio_stream_period_bytes(audio) > PAGE_SIZE) return K_EINVAL;
    xhci_audio_lock();
    if (g_xhci.audio_stream != 0 && g_xhci.audio_stream != audio) {
        xhci_audio_unlock();
        return K_EBUSY;
    }
    g_xhci.audio_stream = audio;
    g_xhci.audio_stream_bound = true;
    g_xhci.audio_stream_running = false;
    g_xhci.audio_stream_queued = false;
    g_xhci.audio_period = 0U;
    g_xhci.audio_frames = 0U;
    xhci_audio_unlock();
    return K_OK;
}
...
kstatus_t xhci_audio_stream_start(struct audio_stream *stream) {
    ...
    bool success = true;
    if (audio == 0 || !g_xhci.initialized || !g_xhci_usb_audio ||
        g_xhci.audio_stream != audio) return K_ENOENT;
    xhci_audio_lock();
    if (g_xhci.audio_stream != audio || !g_xhci.audio_stream_bound) {
        xhci_audio_unlock();
        return K_ENOENT;
    }
    g_xhci.audio_stream_running = true;
    if (g_xhci.audio_stream_queued && !g_xhci.audio_transfer_pending) {
        success = xhci_queue_audio_transfer(&g_xhci);
        if (!success) g_xhci.audio_stream_running = false;
    }
    xhci_audio_unlock();
...
kstatus_t xhci_audio_stream_stop(struct audio_stream *stream) {
    ...
    if (!g_xhci.initialized || !g_xhci_usb_audio) return K_ENOENT;
    xhci_audio_lock();
    if (g_xhci.audio_stream != audio || !g_xhci.audio_stream_bound) {
        xhci_audio_unlock();
        return K_ENOENT;
    ...
    g_xhci.audio_stream_running = false;
    if (!g_xhci.audio_transfer_pending) g_xhci.audio_stream_queued = false;
    xhci_audio_unlock();
    return K_OK;
}
...
kstatus_t xhci_audio_stream_queue(struct audio_stream *stream, uint32_t period,
    ...
    audio_format_t format;
    bool success = true;
    if (audio == 0 || frames == 0U) return K_EINVAL;
    if (!g_xhci.initialized || !g_xhci_usb_audio) return K_ENOENT;
    audio_stream_get_format(audio, &format);
    xhci_audio_lock();
    if (g_xhci.audio_stream != audio || !g_xhci.audio_stream_bound) {
    ...
    if (period >= format.period_count || frames > format.period_frames) {
        xhci_audio_unlock();
        return K_EINVAL;
    }
    if (audio_stream_bytes_for_frames(audio, frames) == 0U ||
        audio_stream_bytes_for_frames(audio, frames) > PAGE_SIZE) {
        xhci_audio_unlock();
        return K_EINVAL;
    }
    g_xhci.audio_period = period;
    g_xhci.audio_frames = frames;
    g_xhci.audio_stream_queued = true;
    if (g_xhci.audio_stream_running) {
        success = xhci_queue_audio_transfer(&g_xhci);
        if (!success) g_xhci.audio_stream_queued = false;
    }
    xhci_audio_unlock();
    return success ? K_OK : K_EIO;
}
...
kstatus_t xhci_audio_stream_disconnect(struct audio_stream *stream) {
    ...
    if (!g_xhci.initialized || !g_xhci_usb_audio) return K_ENOENT;
    xhci_audio_lock();
    if (g_xhci.audio_stream != audio) {
        xhci_audio_unlock();
        return K_ENOENT;
    }
    g_xhci.audio_stream = 0;
    g_xhci.audio_stream_running = false;
    g_xhci.audio_stream_queued = false;
    /* 保持 bound 标志，防止尚未回收的 transfer event 重新启动自测队列。 */
    g_xhci.audio_stream_bound = true;
    xhci_audio_unlock();
    return K_OK;
}
