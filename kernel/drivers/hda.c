...
bool hda_pcm_self_test(void) {
    ...
    if (bdl == 0 || periods == 0) goto cleanup;
    stream = hda_stream_offset(&g_hda, false, 0U);
    if (stream == UINT32_MAX || !hda_alloc_dma(&g_hda, bdl)) goto cleanup;
    for (uint32_t i = 0; i < HDA_PCM_PERIODS; ++i) {
        if (!hda_alloc_dma(&g_hda, &periods[i])) goto cleanup;
    }
    ...
    if (!hda_write32(&g_hda, stream + HDA_SD_CTL, 0U) ||
        !hda_write32(&g_hda, stream + HDA_SD_CBL, PAGE_SIZE * HDA_PCM_PERIODS) ||
        !hda_write8(&g_hda, stream + HDA_SD_LVI, HDA_PCM_PERIODS - 1U) ||
        !hda_write16(&g_hda, stream + HDA_SD_FMT, 0x0011U) ||
        !hda_write32(&g_hda, stream + HDA_SD_BDLPL,
                     (uint32_t)hda_dma_address(bdl)) ||
        !hda_write32(&g_hda, stream + HDA_SD_BDLPU,
                     (uint32_t)(hda_dma_address(bdl) >> 32)) ||
        !hda_write32(&g_hda, stream + HDA_SD_CTL,
                     HDA_SD_CTL_RUN | HDA_SD_CTL_IOCE)) goto cleanup;
    for (uint32_t spin = 0; spin < 100000U; ++spin) {
        uint32_t position = hda_read32(&g_hda, stream + HDA_SD_LPIB);
        if (position != UINT32_MAX && position != 0U) break;
        __asm__ volatile ("pause");
    }
