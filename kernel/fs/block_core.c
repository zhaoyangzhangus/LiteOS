...
static bool block_request_completed(void *context) {
    ...
    unsigned state = atomic_load_explicit(&request->state, memory_order_acquire);
    /* 回调发生在 COMPLETING 阶段，必须等 finalize 写入终态后再返回。 */
    return state == IOREQ_COMPLETED || state == IOREQ_CANCELLED;
}

static void block_request_complete(io_request_t *request) {
    (void)request;
...
kstatus_t block_submit_bio(bio_t *bio) {
    ...
    bio->io->complete = block_request_complete;
    kstatus_t status = io_submit(bio->io);
    if (status == K_OK &&
        atomic_load_explicit(&bio->io->state, memory_order_acquire) == IOREQ_SUBMITTED) {
