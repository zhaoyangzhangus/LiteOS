...
/*
 * Return true when a visible window can affect at least one rectangle in the
 * immutable damage set that this snapshot is about to render.
 *
 * Use the complete outer bounding box here. Rounded corners / titlebar details
 * are intentionally not considered: a conservative false-positive is cheap,
 * while a false-negative would omit pixels required by composition.
 */
static bool compositor_window_intersects_snapshot_damage_locked(
    const window_server_window_t *window,
 ...
 *
 * This is used only for snapshot admission.  The real damage rectangles remain
 * unchanged for raster/publication, so P11 never increases the number of
 * pixels actually rendered or copied to scanout.
 */
...
/*
 * Raw-registry equivalent of compositor_window_fully_covers_damage().
 *
 * The guaranteed opaque rectangle excludes WINDOW_CORNER_RADIUS from every
 * outer edge.  If this rectangle contains the complete snapshot damage bounds,
 * every lower Z-order window is irrelevant to every damage rectangle in the
 * frame.
 */
static bool compositor_window_fully_covers_snapshot_bounds_locked(
    const window_server_window_t *window,
...
/*
 * Find the highest registry window that makes everything below it irrelevant
 * to the complete snapshot damage set.
 *
 * g_window_server.windows[] is bottom -> top, so the first opaque cover found
 * while scanning backwards is the strongest possible floor.
 *
 * The active dragged window is never cut out by this optimization.  Its
 * snapshot view is also used to advance compositor_presented_* after commit.
 */
static uint32_t compositor_snapshot_occlusion_floor_locked(
    const compositor_snapshot_t *snapshot) {
    ...
    for (uint32_t count = g_window_server.count;
         count != 0U;
         --count) {

        uint32_t index = count - 1U;

        if (compositor_window_fully_covers_snapshot_bounds_locked(
...
static bool compositor_snapshot_begin_locked(void) {
    ...
    /*
     * The producer sets drag_blit_valid only when drag_old_x/y names geometry
     * that was actually committed to the retained scene and no scene damage
     * is pending/in flight.  Preserve that proof in the immutable snapshot.
     *
     * If the proof is absent, the existing correctness-first full-new-position
    ...
    /*
     * A non-retained drag is rendered as one complete bounding transaction.
     * Do this before selecting snapshot windows: a window that intersects only
     * the diagonal gap inside that bounding rectangle is still required for
     * correctness.
     *
     * Retained drag deliberately keeps its OLD\NEW / NEW\OLD strips.
     */
    ...
    /*
     * If one upper window's guaranteed opaque interior contains the complete
     * snapshot damage bounds, no lower window can contribute to this frame.
     *
     * P10 already skips non-intersecting windows individually.  P11 adds this
     * Z-order floor so lower windows do not even pay that intersection test,
     * object_get(), view copy or title copy.
     */
    first_window_index =
        compositor_snapshot_occlusion_floor_locked(
            snapshot);

    /*
     * Snapshot only windows that can contribute to this frame.
     *
     ...
     */
    for (uint32_t index = first_window_index;
        ...
        /*
         * Most frames touch only a tiny part of the desktop. Avoid taking a
         * lifetime reference and copying a complete compositor view/title for
         * windows that provably cannot affect any final damage rectangle.
         *
         * Keep the actively dragged window even if it has just moved fully
         * off-screen. snapshot_finish() still needs to advance its
         * compositor_presented_* bookkeeping so a later move can never reuse
         * stale retained geometry.
         */
        if (window->identifier !=
                snapshot->dragging_identifier &&
            !compositor_window_intersects_snapshot_damage_locked(
                window,
                snapshot)) {
            continue;
        }
...
/*
 * P14 QEMU StdVGA hidden-frame publication.
 *
 * The destination is not currently scanned out, so a complete retained scene
 * can be copied there without exposing intermediate rows. The first P14
 * version intentionally refreshes the full hidden frame: a page that was
 * previously visible can contain a software-cursor overlay, and full refresh
 * gives a simple correctness baseline before later incremental page tracking.
 */
static bool compositor_commit_qemu_stdvga(void) {
    ...
    full.left = 0U;
    full.top = 0U;
    full.right = g_window_server.display_width;
    full.bottom = g_window_server.display_height;

    /*
     * Reuse the existing persistent scanline helpers when they are available.
     * They already partition rows contiguously and publish through display
     * core's MOVDIR64B/MOVNTI WC primitive.
     */
    if (!compositor_copy_rect_parallel_buffers(
        ...
        for (uint32_t row = 0U;
             row < g_window_server.display_height;
             ++row) {
    ...
    /* Cursor-only HID updates must follow the newly visible page. */
    g_window_server.framebuffer = front;
    return true;
}
