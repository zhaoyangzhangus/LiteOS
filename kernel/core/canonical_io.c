...
static void io_finalize_owned(io_request_t *req, kstatus_t status,
    ...
    if (req->complete != 0) req->complete(req);
    atomic_store_explicit(&req->state, IOREQ_COMPLETED, memory_order_release);
...
void io_complete(io_request_t *req, kstatus_t status, uint64_t bytes_done) {
    ...
    io_finalize_owned(req, status, bytes_done, true);
}
...
kstatus_t io_submit(io_request_t *req) {
    ...
    if (!atomic_compare_exchange_strong_explicit(&req->state, &expected,
                                                  ...
                                                  memory_order_acq_rel,
                                                  memory_order_acquire)) {
        return expected == IOREQ_COMPLETED ? req->status : K_EBUSY;
    }
    object_get(req);
    ...
    if (!device_io_begin(req->device, req)) {
        ...
        return current == IOREQ_CANCELLED || current == IOREQ_COMPLETED
                   ? req->status
                   : K_EBUSY;
    }
    ...
    if (!atomic_compare_exchange_strong_explicit(&req->state, &queued,
                                                  ...
                                                  memory_order_acq_rel,
                                                  memory_order_acquire)) {
        return queued == IOREQ_CANCELLED || queued == IOREQ_COMPLETED
                   ? req->status
                   : K_EBUSY;
    }
...
kstatus_t io_cancel_with_status(io_request_t *req, kstatus_t completion_status) {
    ...
    for (;;) {
        ...
        if (state == IOREQ_COMPLETED || state == IOREQ_CANCELLED) return K_OK;
