...
void schedule(void) {
    ...
    if (current != 0 && current != cpu->queue.idle &&
        atomic_load_explicit(&current->state, memory_order_relaxed) == THREAD_DEAD &&
        cpu->reap_pending == 0) {
        cpu->reap_pending = current;
    }
    scheduler_unlock(&cpu->queue.lock, queue_flags);
...
void sched_finish_switch(void) {
    ...
    if ((flags & (1ULL << 9)) == 0) return;
    uint32_t cpu_id;
    if (!scheduler_current_cpu(&cpu_id)) return;
    scheduler_cpu_t *cpu = &g_cpus[cpu_id];
    uint64_t queue_flags = scheduler_lock(&cpu->queue.lock);
    thread_t *thread = cpu->reap_pending;
    cpu->reap_pending = 0;
    scheduler_unlock(&cpu->queue.lock, queue_flags);
    if (thread != 0) thread_release_execution_ref(thread);
}
